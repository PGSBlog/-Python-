...
# 배틀크루져 : 공중 유닛, 체력 굿, 공격력 굿!

battlecruiser = FlyableAttacUnit("배틀 크루져", 500, 25, 3)

vulture.move("11시")
# battlecruiser.fly(battlecruiser.name, "9시")
 
# 이렇게 하면 공중, 지상 유닛을 생성할 때 변수를 따로 지정해줘야하는(move, fly) 번거로움이 있다. 
# 위에 FlyableAttacUnit에 move라는 변수를 정의해주면 아래 명령문처럼 사용할 수있다.
battlecruiser.move("9시") # name은 위에 67줄에 선언했기에 self.name은 지정했기에 방향만 FlyableAttacUnit클래스에 절달해주면 된다.

# Unit, AttackUnit, FlyableAttacUnit, Flyable이 있을 때, 

# [지상 유닛] AttackUnit(move정의)--->Unit

# [공중 유닛] FlyableAttacUnit(지상 유닛에서 사용한 move함수 재 정의) ---> Flyable
#                                                                   ---> AttackUnit
